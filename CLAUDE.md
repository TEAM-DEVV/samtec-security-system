...
- **Phase 4 (payroll) — Samuel owns it, end to end. The contract and the mock API are in**, with the hand-calculated payslips pinned as tests (`apps/web/src/mocks/handlers/payroll.ts`, `data/payroll.ts`). **The eight tables, the migration and the calculation are in too**, with the hand-calculated payslips as tests on both sides and the rules the database enforces proved against a real PostgreSQL (`apps/api/test/payroll-rules.e2e-spec.ts`). **The whole payroll API is in**: the months, the tax table versions, pay terms and payment details; calculating a run and reading it back; submit, approve, reject and mark paid; the payslip PDF (written directly, no library — see the module README); and the bank file. **Left:** only the dashboard screens. Every design question is decided in [docs/plan/09-payroll-engine-ghana.md](docs/plan/09-payroll-engine-ghana.md) — decisions 22 to 26 were added during the build after the four-lens reviews, and 25 corrects 22; the house style is in [docs/plan/16-building-a-backend-module.md](docs/plan/16-building-a-backend-module.md).
