...
Phase 0 is merged. **The Phase 1 backend is complete**: sign-in, two-factor, the audit log, employees (read, create, update, terminate), employment periods, sites, posts, shift patterns and user management are built. Samuel's screens complete the frontend side: the Users pages (ADMIN only: `apps/web/src/pages/users-page.tsx`, `new-user-page.tsx`, `user-detail-page.tsx`), the change-password form (`change-password-page.tsx`, at `/account/password`) and the public `/set-password` page (`set-password-page.tsx`) are built. One-time links are `<dashboard>/set-password#token=<token>`; the token sits after `#` so it never reaches a server log. **The Phase 2 (attendance) backend is complete:** devices, signed ingest, pairing into work segments, the exception queue and resolving it, and a device simulator for the exit demo ([docs/guides/10-attendance-demo.md](docs/guides/10-attendance-demo.md)). The dashboard's Attendance day view, a guard's "My attendance", the exception queue with its resolution screen and the Devices page (ADMIN) can be built against the mock API or the real one; [docs/plan/12-attendance-design.md](docs/plan/12-attendance-design.md) explains every rule. **Phase 3 (biometrics) is being built** from [docs/plan/13-biometrics-design.md](docs/plan/13-biometrics-design.md): workers clock in by face (and the device's own fingerprint sensor) on a company kiosk at the site, and ZKTeco terminals come through a gateway. Francis builds the API, the kiosk app (`apps/kiosk`) and the gateway (`apps/gateway`). The contract and the mock API are in, so the dashboard's Phase 3 screens can be built now: the live clock-ins board (`GET /attendance/punches`, refreshed every 5 seconds, flagged methods in amber), the employee Biometrics panel, the duplicate-enrollment queue, kiosk attempts per device, and the new device fields. One owner task outside the repository remains (repository protection settings); the ZKTeco device waits for a paying client. See `docs/plan/07-roadmap.md`.
