...
- **Phase 4 (payroll) — Samuel owns it, end to end. The contract and the mock API are in**, with the hand-calculated payslips pinned as tests (`apps/web/src/mocks/handlers/payroll.ts`, `data/payroll.ts`). **Left:** the tables and migration, the calculation in `apps/api`, the endpoints, the payslip PDF and the screens. Every design question is decided in [docs/plan/09-payroll-engine-ghana.md](docs/plan/09-payroll-engine-ghana.md) — decisions 22 to 24 were added during the build after the four-lens review; the house style is in [docs/plan/16-building-a-backend-module.md](docs/plan/16-building-a-backend-module.md).
- **Phase 5 (ghost detection) — Francis owns it, end to end.** Not started, and it merges **after** payroll because its rules read payroll data.
