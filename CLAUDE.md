...
- `apps/kiosk`: the phone at the gate. React 19 + Vite 8 and **nothing else** — plain CSS, no router, no component library, because it must start fast on a cheap Android phone. Read [its README](apps/kiosk/README.md) first: the device signing scheme and the liveness challenge are both easy to get subtly wrong.
- `packages/contracts`: `openapi.yaml`, the API contract, and the generated types package `@samtec/contracts`. Never edit `src/generated/`. The one runtime value in it is the shared device-signature example, at its own subpath.
...
| Kiosk on a pretend camera (port 5174) | `pnpm dev:kiosk` |
...
| The same backup on a schedule, keeping the newest 14 | `pnpm --filter @samtec/api db:backup:scheduled` |
| Measure a burst of punches through the signed endpoint | `pnpm --filter @samtec/api load:punches` |
| Measure what each face number would decide ([report](docs/guides/14-face-threshold-report.md)) | `pnpm --filter @samtec/api face:scores` |
...
- **Phase 3 (biometrics) — the kiosk API is complete**, from [docs/plan/13-biometrics-design.md](docs/plan/13-biometrics-design.md): consent, enrollment, the duplicate-enrollment queue, identify/confirm/"Not me", a supervisor's co-sign, and fingerprints on the kiosk's own sensor (`FACE_PASSKEY`, plus the staff-number fallback `STAFF_PASSKEY`). Screens are built: the live clock-ins board (`live-board-page.tsx`), the employee Biometrics panel (`components/biometrics-panel.tsx`), the duplicate queue (`duplicate-faces-page.tsx`) and kiosk attempts (`kiosk-attempts-page.tsx`). The three gateway endpoints (`POST /ingest/roster`, `POST /ingest/enrollments`, `POST /devices/{id}/finger-enrollment-windows`) are built and covered by `apps/api/test/gateway.e2e-spec.ts`. **The kiosk app has started** (`apps/kiosk`, Samuel): set-up by pasting the device secret from the Devices page, and the everyday clock-in and clock-out screen — the head-turn challenge, the name for two seconds with a "Not me" button, and the three-failure fallback notice. It runs on a pretend camera (`pnpm dev:kiosk`), so it works on any machine; the face engine sits behind a `FaceEngine` seam so the real Human models drop in without a screen changing. The signing scheme is pinned to the server's by one shared worked example that both sides assert. **Left:** the kiosk's consent, enrollment and save-a-finger screens, the two fallbacks, the real Human engine, and the gateway (`apps/gateway`) — all Samuel's.
- **Phase 4 (payroll) — Samuel owns it, end to end. The contract and the mock API are in**, with the hand-calculated payslips pinned as tests (`apps/web/src/mocks/handlers/payroll.ts`, `data/payroll.ts`). **The eight tables, the migration and the calculation are in too**, with the hand-calculated payslips as tests on both sides and the rules the database enforces proved against a real PostgreSQL (`apps/api/test/payroll-rules.e2e-spec.ts`). **The whole payroll API is in**: the months, the tax table versions, pay terms and payment details; calculating a run and reading it back; submit, approve, reject and mark paid; the payslip PDF (written directly, no library — see the module README); and the bank file. **The screens are built too**: the months and runs (`payroll-page.tsx`), one run with its lines and the approve, reject or mark-paid decision (`payroll-run-page.tsx`), and a worker’s own payslips (`my-payslips-page.tsx`). **Phase 4 is complete.** Every design question is decided in [docs/plan/09-payroll-engine-ghana.md](docs/plan/09-payroll-engine-ghana.md) — decisions 22 to 27 were added during the build after the four-lens reviews, 25 corrects 22, and 27 closed a real hole in rule R3 that minutes alone could not see; the house style is in [docs/plan/16-building-a-backend-module.md](docs/plan/16-building-a-backend-module.md).
...
- **Phase 7 (hardening) — Francis owns it. Started.** The daily detection run is a Vercel Cron entry (no secret to set). **Two administrators:** creating, promoting, resetting or switching on an ADMIN account holds it as `AWAITING_CONFIRMATION` until a different ADMIN calls `POST /users/{id}/confirm-admin` — rules in [docs/plan/06-security-and-review-gates.md](docs/plan/06-security-and-review-gates.md), "Two administrators". A device key is born switched off too, and whoever issued it may not switch it on. **Load test** (`apps/api/scripts/load-test.ts`, `pnpm --filter @samtec/api load:punches`) and **backup and restore** (`apps/api/scripts/backup.ts`, written up in [docs/guides/11-backup-and-restore.md](docs/guides/11-backup-and-restore.md)) are both done, and the threat model is refreshed. The **whole-system four-lens review** is done too: twenty findings, all fixed — see [docs/plan/06-security-and-review-gates.md](docs/plan/06-security-and-review-gates.md). A weekly schedule takes a backup on its own (`pnpm --filter @samtec/api db:backup:scheduled`, a Windows task on Francis's computer, Sundays at 13:00). **Phase 7 is complete.**
- **Phase 8 (deploy and present) — Francis owns it. Started.** The **defence pack** is written: [The defence pack](docs/guides/13-defence-pack.md) explains the whole system in plain language — the thirty-five tables, then every module, then every screen — for Samuel to learn and defend, with each gap boxed and named against its owner. **Left:** the demo environment and production settings, the presentation, and Francis's final visual pass over every screen (the last build step of the project, after Samuel's Phase 6).
- **Phase 6 (reports) — Samuel’s half is done.** The key figures and both CSV downloads (`reports-page.tsx`, `apps/api/src/modules/reports/`), the payroll run summary PDF, and a worker’s own payslips. The reports module owns no tables: every figure is counted from the tables the screens already read, so a report can never disagree with the page beside it. **Left:** Francis’s final visual pass.
- **Phases 7–8.** Reports and the guard's own payslip: Samuel. The final visual pass, hardening and the defence pack: Francis. Both present.
