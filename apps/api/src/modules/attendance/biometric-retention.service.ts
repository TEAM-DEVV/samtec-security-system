...
export class BiometricRetentionService {
  ...
  private async wipeFaces(
    ...
    // and every extra round trip is time somebody else waits. A leaver
    // stays TERMINATED; this only moves somebody still waiting back to
    // PENDING_ENROLLMENT.
    await this.employees.clearBiometricsEnrolledMany(companyId, wiped, tx);
  }
