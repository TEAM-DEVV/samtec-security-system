...
| One insider activating a ghost without a face | Insider (ADMIN) | Every exemption (a withdrawal of consent only files one) and every collision decision needs a second ADMIN who did not act on the worker; one open question at a time; a revoke cannot wipe away an open review; withdrawing never activates anyone; a record blocked as a duplicate can only be terminated; rule R11 flags decisions with indirect links (Phases 3 and 5) |
