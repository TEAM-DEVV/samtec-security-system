...
| `biometric_credentials` | employee, kind (`FACE`, `TERMINAL_FINGER`), device, the **encrypted** face template, key version, enrolled by, duplicate-check result (`PASSED`, `COLLISION`, `CLEARED`, `NOT_CHECKED`), who it looked like and how closely, the review (open until decided: the verdict, the record kept, the note, who decided), status (`PENDING`, `ACTIVE`, `BLOCKED`, `REVOKED`), `wiped_at` | Never deleted; a wipe empties the template but keeps who it looked like. At most one unwiped face per employee (a partial unique index). A new face is enrolled on a kiosk, undecided, and only with the worker's own consent, still given. A face that collided is never matched until a second ADMIN clears it. The record that loses a SAME_PERSON decision keeps a blocked face and no face or fingerprint key in use, checked when the change is saved. A face blocked as a duplicate stays `BLOCKED` for good, and a blocked record gets nothing new |
| `biometric_exemptions` | employee, status (`REQUESTED`, `APPROVED`, `REJECTED`, `ENDED`), reason code (`DECLINED`, `CANNOT_ENROLL`, `CONSENT_WITHDRAWN`), a short note, who asked and when, who decided and when | Never deleted. A request is decided once, by a different ADMIN, while it is still waiting |
| `device_passkeys` | employee, device, credential ID, public key, signature counter, "synced" flag, `revoked_at` | Never deleted. One live key per employee per device, and only on a kiosk with fingerprints switched on. The counter only goes up; a revoked key is never used again |
