...
4. **Face accuracy on Ghanaian faces is unpublished.** The margin rule, the "Not me" button and Phase 4's maker–checker payroll limit the harm of a false match. Twins may need the finger or a co-sign.
   - **Measured, 2026-09-25** ([the threshold report](../guides/14-face-threshold-report.md) section 6): the twin case is worse than "may need". Two faces scoring 0.75 against each other, where the same person scores 0.78, give 5 wrong matches in 200 at `ft-1`, and no value of `match` or `lead` fixes it at a price worth paying — closing it needs `lead` 0.15, which fails one honest guard in five. Enrollment does catch such a pair (all 8 near-twins in the study were held for a second ADMIN), **so the fix is to require the finger or the staff number for a pair an ADMIN closed as `DIFFERENT_PEOPLE`.** Not built; it is recommendation 1 of that report.
   - The unpublished part stands, and the report names it as its own biggest gap: a generated study cannot say anything about skin tone, age or sex, so the pilot must be built to answer that.
5. **More guards means more false collisions:** the chance is 1 − (1 − FMR)^N. At a 0.1% false-match rate and 500 guards, about 39% of enrollments would need a review, so the pilot sets the threshold.
   - **Measured, 2026-09-25** ([the threshold report](../guides/14-face-threshold-report.md) section 7): the trend is real and shows up exactly as predicted — 0% of honest new starters queried at 40 people on file, 11% at 100, 21.5% at 200. Only the trend generalises to real faces, not the fractions. The room to raise the threshold is thinner than it looks, though: the first ghost escapes at 0.65, not 0.70.
