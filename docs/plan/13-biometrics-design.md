...
- The sweep switches off the fingerprint keys of anyone past their termination date, and wipes the face of anyone terminated at least 90 days ago. It also wipes a face that has waited 90 days for a duplicate review or for an abandoned hire; the review stays open, because only a second ADMIN may close it. For leavers, it clears the exemption note and the attempts' network addresses after the same 90 days (the reason codes and the rows stay). It handles at most 50 people per heartbeat; the next heartbeat carries on.
...
**COLLISION.** The worker stays pending, and the kiosk shows only "Needs an admin review" (never who they looked like). A **second ADMIN** checks both people's Ghana Cards in person and records a verdict with a note. It may never be the ADMIN who did the enrollment, or whoever created either employee record.
...
- `SAME_PERSON`: one person with two records. The reviewer says which record belongs to the person whose card they checked (`keepEmployeeId`). The choice matters. Suppose an insider wipes a real guard's face and enrolls a ghost with it. When the real guard enrolls again, the reviewer keeps the real guard's record and blocks the ghost, instead of always blocking the newer face.
  ...
  - The decision is final. The blocked record can only be terminated: consent, enrollment and exemption are refused for it.
...
**One open question at a time.** An open duplicate review, an exemption request waiting, or a face blocked as a duplicate each stop consent, enrollment and new exemption requests (`409`) until they are settled. Every question has exactly one way out, and it goes through a second ADMIN.
...
- A **different** ADMIN checks the Ghana Card in person and approves or rejects it (`POST /employees/{id}/biometric-exemption/review`). It may never be the ADMIN who asked, whoever created the employee record, or anyone who enrolled a face for this worker. The rules are checked again at approval, not only when someone asked.
- Once approved, the worker is `ACTIVE` without a face and clocks in only by a supervisor's co-sign, so every hour they work is flagged. If the worker later enrolls a face that passes, the exemption ends (`ENDED`).

**Withdrawing consent** (`POST /employees/{id}/biometric-consents/withdraw`) wipes the face at once and switches off the keys. It never changes the employee's status.

- If the face was in use (`ACTIVE`, so it had passed the duplicate check) and the worker is not terminated, the worker is exempted at once (`CONSENT_WITHDRAWN`) and keeps working with co-signed clock-ins. An exemption already approved is kept.
- Any other face (waiting for review, or blocked) gives no exemption, and an open review stays open. **Withdrawing never activates anyone.**
...
- It unlocks only when the last 3 `CLOCK` attempts on this device all failed (a match cancelled with "Not me" counts as a failure; co-sign attempts do not count), the newest under 2 minutes old.
- **Every call uses up the unlock**, whatever the answer, and is recorded with the staff number tried, so one unlock can never be used to try staff numbers one after another.
...
| `POST /employees/{id}/biometric-exemption/review` | Access token | ADMIN, never the one who asked, created the record or enrolled a face |
| `POST /employees/{id}/biometric-consents/withdraw` | Access token | ADMIN, HR_PAYROLL |
| `GET /biometric-collisions`, `POST /biometric-collisions/{credentialId}/resolve` | Access token | ADMIN, never the one who enrolled or created either record |
...
| 4 | Kiosk sign-in (`KIOSK_ORIGINS`, no refresh cookie, kiosk-only token), consent, enrollment, collisions, revoke, withdraw, the two-ADMIN exemption, retention sweep | The same face twice → COLLISION; parallel enrollments caught; resolver ≠ enroller; approver ≠ asker; a revoke during a review → 409; withdrawal never activates; a suspended worker stays suspended; overlapping origin lists stop the API; clean logs |
| 5 | Identify, confirm, "Not me", co-sign, the live clock-ins board | A resend → `DUPLICATE`; the margin rule; no scores returned; one co-sign makes one punch, on its own device only; confirm refuses a co-sign |
...
- the duplicate-enrollment queue (open and resolved), where the note is required, the enroller cannot decide, and `SAME_PERSON` asks which record to keep;
