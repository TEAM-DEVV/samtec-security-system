...
| `biometric_credentials` | employee, kind (`FACE`, `TERMINAL_FINGER`), device, the **encrypted** face template, key version, enrolled by, duplicate-check result (`PASSED`, `COLLISION`, `CLEARED`, `NOT_CHECKED`), who it looked like and how closely, the review (`OPEN`, `RESOLVED` or `VOID`, the verdict, the record kept, the note, who decided), status (`PENDING`, `ACTIVE`, `BLOCKED`, `REVOKED`), `wiped_at` | Never deleted; a wipe empties the template but keeps who it looked like. One ACTIVE face per employee |
| `biometric_exemptions` | employee, status (`REQUESTED`, `APPROVED`, `REJECTED`), reason code (`DECLINED`, `CANNOT_ENROLL`, `CONSENT_WITHDRAWN`), a short note, who asked and when, who decided and when | Never deleted. A request is decided once, by a different ADMIN |
...
1. **The ADMIN signs in on the kiosk** with password and two-factor. The API knows the dashboard's own addresses (`CORS_ORIGINS`), and a sign-in from any other address is a **kiosk sign-in**. That rule holds for all three sign-in steps (`/auth/login`, `/auth/2fa/verify` and `/auth/2fa/enable`).
   - It gets **no refresh cookie**, so the kiosk never stays signed in: the 15-minute access token lives in memory until 5 idle minutes pass or the ADMIN taps "Done".
   - Its token works **only for the kiosk screens**: `GET /auth/me`, `GET /sites`, `GET /employees` (the list has no Ghana Card numbers), `POST /devices` for a `FACE_KIOSK`, the consent text and the kiosk's ADMIN routes. Everything else answers `403`.
...
2. **Pick the worker** and type the **last 4 digits of the Ghana Card** they are holding. Workforce checks them, and the full number never reaches the kiosk.
   ...
   - After 5 wrong answers for one worker within an hour, that worker waits for the rest of the hour (`429`).
...
- `SAME_PERSON`: one person with two records. The reviewer says which record belongs to the person whose card they checked (`keepEmployeeId`). The other record's face is wiped (`BLOCKED`), and HR investigates that record as a possible ghost. The choice matters. Suppose an insider wipes a real guard's face and enrolls a ghost with it. When the real guard enrolls again, the reviewer keeps the real guard's record and blocks the ghost, instead of always blocking the newer face.

If the face is wiped before anyone decides (a revoke, a withdrawal, or 90 days waiting), the review becomes `VOID`: it can no longer be decided, and who it looked like stays on record. Phase 5's rule R1 (duplicate biometrics) reads these rows, so no extra alerts table is needed.

**Refusing consent.** An exemption takes **two ADMINs**, like the collision review:

- One ADMIN asks (`POST /employees/{id}/biometric-exemption`). This is only for a worker still `PENDING_ENROLLMENT` with no open review.
...
- A **different** ADMIN checks the Ghana Card in person and approves or rejects it (`POST /employees/{id}/biometric-exemption/review`).
- Once approved, the worker is `ACTIVE` without a face and clocks in only by a supervisor's co-sign, so every hour they work is flagged.

**Withdrawing consent** (`POST /employees/{id}/biometric-consents/withdraw`) wipes the face at once, switches off the keys and voids any open review.

- A worker who was `ACTIVE` already passed the duplicate check. They stay `ACTIVE` and are exempted at once (`CONSENT_WITHDRAWN`), so they keep working with co-signed clock-ins.
- A worker still `PENDING_ENROLLMENT` stays pending. **Withdrawing never activates anyone.**
- A worker who withdrew may consent again later.
...
For a co-sign, the worker types their staff number. A supervisor then passes identify with purpose `CO_SIGN`, **that staff number** and the direction.

- The supervisor must be ACTIVE, assigned to this site, and not the worker.
- The worker must be ACTIVE and posted to this site.
- `POST /kiosk/assisted-punches` then makes **one** `PIN_FALLBACK` punch, with the reason audited. The punch's ID comes from the co-sign attempt, so a co-sign can never be reused for a second punch or a different worker.
...
- It unlocks only after 3 failed face attempts **in a row** on this device, the last one under 2 minutes ago.
- Each unlock allows one fallback.
...
| `POST /auth/login`, `/auth/2fa/verify`, `/auth/2fa/enable`: a sign-in from outside the dashboard's addresses gets no refresh cookie and a kiosk-only token | Public | The kiosk operator |
...
| `POST /employees/{id}/biometric-exemption/review` | Access token | ADMIN, never the one who asked |
...
| 4 | Kiosk sign-in (no refresh cookie, kiosk-only token), consent, enrollment, collisions, revoke, withdraw, the two-ADMIN exemption, retention sweep | The same face twice → COLLISION; parallel enrollments caught; resolver ≠ enroller; approver ≠ asker; withdrawal never activates; clean logs |
| 5 | Identify, confirm, "Not me", co-sign, the live clock-ins board | A resend → `DUPLICATE`; the margin rule; no scores returned; one co-sign makes one punch |
...
- a Biometrics panel on the employee page: consent, face status, fingerprint keys and exemption, with the actions revoke, withdraw, ask for an exemption, and approve or reject one (the ADMIN who asked cannot decide it);
- the duplicate-enrollment queue (open, resolved and void), where the note is required, the enroller cannot decide, and `SAME_PERSON` asks which record to keep;
...
1. **The kiosk is trusted.** Browser anti-spoofing is prototype-grade. It stops printed photos and screens held up to the camera, but not a replayed video, a mask or a modified kiosk.
   ...
   - What limits the harm: the key only works on the kiosk routes; a worker with a fingerprint key on that kiosk still needs the finger; every attempt records its network address, so Phase 5 can flag a kiosk key used from two places; and the ADMIN can rotate the secret.
...
8. **One ADMIN can create and enroll a fake person with an accomplice's face** (a face nobody else has enrolled). Both steps are audited, Phase 5 flags it, and probing shows in the attempt log. Activating someone *without* a face always takes two ADMINs.
