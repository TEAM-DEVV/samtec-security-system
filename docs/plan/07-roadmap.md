...
- [x] Dashboard (Samuel): live clock-ins board (`/attendance/live`), the employee Biometrics panel, the duplicate-enrollment queue (`/biometrics/duplicates`), kiosk attempts per device (`/devices/attempts`). Built against the mock API; the new device fields (serial number and the kiosk fingerprint switch) shipped with the Phase 2 Devices pages.
