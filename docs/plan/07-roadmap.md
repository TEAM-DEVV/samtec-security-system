...
- [x] Dashboard (Samuel): the employee forms — register somebody (`/employees/new`), change their record (`/employees/:id/edit`) and record that they have left (`/employees/:id/terminate`, its own page, because it is the one thing here that cannot be undone). The Ghana Card number and the start date are read-only when editing, and only the fields that actually changed are sent
...
- [x] **The threshold report** — [The face-matcher threshold report](../guides/14-face-threshold-report.md), produced by `pnpm --filter @samtec/api face:scores`. It measures what each of the five `ft-1` numbers would decide, using the shipped matcher rather than a copy, and it found three things. First, the lead rule earns its place: on a crowd with look-alikes, removing it matches four clock-ins in two hundred to the **wrong person**, and 0.05 turns all four into a second try. Second, and this one changed the verdict — **near-twins defeat the thresholds.** Two people scoring 0.75 against each other, where the same person scores 0.78, give five wrong matches in two hundred at `ft-1`, and no value of `match` or `lead` fixes it at a price worth paying (closing it needs `lead` 0.15, which fails one honest guard in five). The answer is not a number: the duplicate queue already stops all eight of those people at enrollment, so the fix is to require a second factor for a pair an administrator closed as `DIFFERENT_PEOPLE` — the fingerprint and the staff number already exist. Third, the duplicate queue's false-query rate grows with headcount (0% at 40 people on file, 21.5% at 200), and the room above 0.60 is thin because the first ghost escapes at 0.65. **Still owed, and said plainly in the report:** the pilot has not happened, so the numbers come from a generated stand-in; and nothing here says how the matcher behaves across skin tones, ages or between men and women, which for a Ghanaian workforce is the most important untested question of the three
...
- [x] The screens: the months and their runs, one run with every line and the decision about it, and a worker’s own payslips with the PDF download
...
- [x] **Full security review of the repository**, in the same pass: its finding was that the Vercel build applied migrations on every deployment, not only the production one. Checking the dashboard showed the trap is latent rather than live — Branch Tracking is off for `samtec-test-api`, so only `main` has ever deployed — and it is now guarded anyway, because one toggle or one command-line deploy would have applied an unreviewed migration to the shared database
...
- [x] **Backup and restore drill**, written up with its numbers in [Backup and restore](../guides/11-backup-and-restore.md). It found that the hosted database has **no backups at all** (Supabase free plan), so the drill also built the backup: 17,535 rows out, database destroyed, rebuilt empty, put back, and the whole test suite run against the restored copy. A weekly schedule now takes one without being asked (`db:backup:scheduled`, a Windows task on Francis's computer, proven, keeping the newest 14 — about three months — with a log). **Still owed:** a week is the most it can lose, which is right for TEST and wrong for real pay; the schedule is only as reliable as one laptop; and the full drill has not been run against the hosted database
