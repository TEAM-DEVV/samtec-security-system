...
| Heading font | **Bricolage Grotesque**, bundled with `@fontsource-variable/bricolage-grotesque` | The display face of the build map, so the product and its plan share one look. Bundled like Geist: no outside font request, and the strict Content-Security-Policy stays as it is. |
...
| 2026-09-24 | The daily ghost-detection run uses **Vercel Cron** (an entry in `apps/api/vercel.json`) calling a public `GET /detection/daily-sweep`, rather than a separate scheduler or `pg_cron` | The schedule belongs to the project that already hosts the API, so it deploys with the code and there is nothing to set up by hand. No shared secret: the route can cause at most the one sweep per company per day it exists to cause, and answers "nothing due" from memory for a minute (docs/plan/08 §7). `pg_cron` would put application logic in the database, which nothing else here does. |
...
| 2026-09-25 | **Decided against `pdfkit` for the payslip PDF** and wrote the generator by hand (`apps/api/src/modules/payroll/payslip-pdf.ts`, about 300 lines with 24 tests) | The plan named pdfkit before anybody checked it. It has no provenance attestation on any version, and nor do its six runtime dependencies, so pnpm's `trustPolicy: no-downgrade` lets it through only because its trust has never been better — and it would add roughly 10 MB to a serverless function. A payslip is one page of text in a standard font: PDF is a text format, the fourteen standard fonts need no embedding, and using Courier for the money column makes the amounts right-align by counting characters, so no font metrics are needed either. That is the TOTP case, not the Reed–Solomon case. It is one function behind one interface, so a library can replace it later without touching anything else. |
...
| 2026-09-23 | Added **@fontsource-variable/bricolage-grotesque** to the dashboard, and pulled the visual level-up forward from Phase 6 as a design foundation: heading font, motion tokens with a reduced-motion switch, a boot screen, the sign-in stage, the shell and the page styling | Samuel asked for the executive look now, so every page built from here inherits it instead of being restyled at the end. A font-only package with no install scripts, served from our own build like Geist, and it passed pnpm's supply-chain policy. Phase 6 keeps the final review pass over every screen. |
