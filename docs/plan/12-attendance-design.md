...
- Every request is signed: `HMAC-SHA256(secret, "v1\n<timestamp>\n<route>\n<body>")`. `<route>` is a fixed name (`ingest/punches` or `ingest/heartbeat`), not the URL, so hosting rewrites can never break it and a signature can never be moved to the other endpoint.
