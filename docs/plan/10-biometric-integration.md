...
- **Enroll:** capture 3 frames and store the embedding, encrypted, on the server. **Identify:** the API decrypts the stored embeddings and compares the new one with each, using Human's own similarity formula. That formula is based on Euclidean distance, not cosine similarity; it is copied to the server in `apps/api/src/modules/attendance/face-match.ts` (built), with its numbers in `face-thresholds.ts`, so the kiosk and the server always agree. A vector index such as pgvector would need unencrypted embeddings, so it waits until a company has more than about 300 guards.
